    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.transform:
                ...
                image = torch.tensor(image).unsqueeze(0)  # fallback
            return image
...
# データセットのパスを別々に指定
parser.add_argument("--train_data_dir", type=str, default='/app/data/train')  # 学習データのパス
parser.add_argument("--val_data_dir", type=str, default='/app/data/validation')  # 検証データのパス
...
print(torch.version.cuda)  # PyTorch がサポートしている CUDA バージョン
print(torch.cuda.is_available())  # GPUが使えるか確認
print(torch.cuda.current_device())  # 使用中のデバイスID
print(torch.cuda.get_device_name(0))  # デバイス名を表示
...
transform = transforms.Compose([
    transforms.ToPILImage(),  # 将数据转换为PIL Image
    transforms.Resize((256, 256)),  # 调整图像大小
    transforms.ToTensor()           # 转换为张量
])
# DICOMデータセットの読み込み（学習用）
train_dataset = DICOMDataset(data_path=args.train_data_dir, transform=transform)
# DICOMデータセットの読み込み（検証用）
val_dataset = DICOMDataset(data_path=args.val_data_dir, transform=transform)
